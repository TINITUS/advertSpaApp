advertsApp.controller('UserDeleteAdCtrl', ['$scope', '$routeParams', '$location', 'categoriesData', 'townsData', 'authService','userAdsService', 'growl', function ($scope, $routeParams, $location, categoriesData, townsData, authService, userAdsService, growl) {
        $scope.pageTitle = "Detete ad";
        $scope.imagePlaceholder = "data:image/jpeg;base64,";
        $scope.user = (authService.getUserData() != null || authService.getUserData() != undefined) ? authService.getUserData().username: null;
        $scope.logOut = function(){
            userData.userLogOut().then(
                function (data){
                    authService.removeUserData();
                    growl.addSuccessMessage('Logged-out successfully!');
                    $scope.user = null;
                    $location.path('/');
                },
                function (error){
                    console.log(error);
                }
            );
        };

        userAdsService.getById($routeParams.id)
            .then(
            function success(adData) {
                $scope.ad = adData;
                categoriesData.getCategories().$promise.then(
                    function (categories) {
                        angular.forEach(categories, function (category, index) {
                            if (category.id == adData.categoryId) {
                                $scope.ad.category = category.name;
                            }
                        });
                    }
                );

                townsData.getTowns().$promise.then(
                    function (towns) {
                        angular.forEach(towns, function (town, index) {
                            if (town.id == adData.townId) {
                                $scope.ad.town = town.name;
                            }
                        });
                    }
                );
            },
            function error(err) {
                console.log(err);                
                growl.addErrorMessage('Advertisment data couldn\'t be received');
            }
        );

        $scope.deleteUserAd = function (ad) {
            userAdsService.deleteUserAd(ad)
                .then(
                function adDeleteSuccess(adDeleteData) {
                    growl.addSuccessMessage('Advertisement deleted successfully.');
                    $location.path('/user/ads');
                },
                function adDeleteError(adDeleteErr) {
                    growl.addErrorMessage('Advertisement couldn\'t be deleted.');
                    //$location.path('/user/ads');
                    console.log(adDeleteErr);
                }
            )
        }
    }]);