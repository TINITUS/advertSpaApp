advertsApp.controller('UserAdsCtrl', ['$scope','$location', 'userAdsService', 'authService', 'userData', 'growl', function($scope,$location, userAdsService, authService, userData, growl){
	$scope.totalItems = 0;
	...
	$scope.deactivateAd = function (ad) {
        userAdsService.deactivate(ad.id)
            .then(
            function adDeativateSuccess(adDeactivateData) {
                growl.addSuccessMessage('Ad was deactivated successfully');
                $scope.getAds($scope.page);
            },
            function adDeactivateError(adDeactivateError) {
                console.log(adDeactivateError);
                growl.addErrorMessage('Ad couldn\'t be deactivated.');
            }
        )
    };

    $scope.publishAgain = function (ad) {
        userAdsService.publishAgain(ad.id)
            .then(
            function publishAgainSuccess(publishAgainData) {
                growl.addSuccessMessage('The ad was published successfully again.');
                $scope.getAds($scope.page);
            },
            function publishAgainError(publishAgainError) {
                console.log(publishAgainError);
                growl.addErrorMessage('The ad couldn\'t be published again.');
            }
        )
    };
